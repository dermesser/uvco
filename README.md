...
This is generally the case in a deadlock-like situation: if the loop weren't terminated, the
application would hang, as there is no code scheduled to run, and no possibility of code being
scheduled to run at any later point.
...
[gcovr](https://github.com/gcovr/gcovr), which should be installed. Alternatively, use `make grcov`
in order to use the [`grcov`](https://github.com/mozilla/grcov) tool. The coverage html is in
